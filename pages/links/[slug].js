...
const Links = ({
  ...
  const loadPopular = async () => {
    const response = await axios.get(
      `http://localhost:8000/api/link/popular/${category.slug}`
    )
  ...
  const handleClick = async (linkId) => {
    const response = await axios.put('http://localhost:8000/api/click-count', {
      linkId,
    })
    loadPopular()
  }
  ...
  const loadUpdatedLinks = async () => {
    const response = await axios.post(
      `http://localhost:8000/api/category/${query.slug}`
    )
